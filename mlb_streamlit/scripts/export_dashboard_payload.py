...
cache files for consumption by the React CDN dashboard.
...
# Add the project root to sys.path so we can import the cache module.
...
def _games_by_date(games: list[dict]) -> dict[str, list[dict]]:
    """Group games by their date field."""
...
def _game_cards(games: list[dict]) -> dict[str, dict]:
    """Index games by their gamePk for the calibration view."""
    return {str(g["gamePk"]): g for g in games if "gamePk" in g}


def _compute_moneyline_metrics(games: list[dict]) -> dict:
    """Compute moneyline total/correct/accuracy from completed games."""
    completed = [
        g for g in games
...
def _compute_totals_metrics(games: list[dict]) -> dict:
    """Compute totals (run projection) metrics from completed games."""
    t_n = 0
    t_abs = 0.0
    t_sq = 0.0
    t_bias = 0.0
...
def _compute_runline_metrics(games: list[dict]) -> dict:
    """Compute run-line (±1.5) metrics from completed games."""
    preds: list[float] = []
    labels: list[int] = []
    for g in games:
