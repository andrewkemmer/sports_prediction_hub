"""Static MLB team metadata and ballpark factors (port of src/convex/ml/teams.ts).

Keyed by MLB Stats API team id. Used to add abbreviations / short names
without the verbose `team` hydrate.
"""
...
def team_meta(team_id: int) -> dict:
    """Return the team metadata dict (abbrev / name / fullName / color)."""
    return TEAMS.get(team_id) or {
        "abbrev": "TBD",
        "name": f"Team {team_id}",
...
# (league average = 1.00). Coors Field is the most hitter-friendly park.
